                ...
                <tbody>
                  {buggy_position.map((bug, index) => (
                    <tr key={index}>
                      ...
                      <td className="border p-2"> <Checkbox checked={selectedBugs.includes(bug.id)}
                    onCheckedChange={() => handleCheckboxChange(bug.id)}/> </td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <p className="text-gray-500 mb-2 italic">
                Hãy tick vào các suggestion mà bạn đã sử dụng
              </p>
              <Button onClick={handleBugCheck} className="mt-4 px-4 py-2 rounded-xl border-1 text-white bg-zinc-800 hover:bg-gray-200 hover:text-black transition duration-300 ease-in-out shadow">
                Submit feedback
              </Button>
            </div>
            </div>
